...
namespace Xamarin.Forms.Core
{
    ...
    public class EncryptionService : IEncryptionService
    {
        ...
        public byte[] GetHash(string input, Encoding encoding)
        {
            if (null == input)
        ...
        public string GetHashString(string input, Encoding encoding)
        {
            if (null == input)
                throw new System.ArgumentNullException("input", "Unable to calculate hash over null input data");
            if (null == encoding)
        ...
        internal byte[] GetHashFinalBlock(byte[] input, int ibStart, int cbSize, ABCDStruct ABCD, Int64 len)
        {
		...
		//    D = 0x10325476;
		*/
        internal void GetHashBlock(byte[] input, ref ABCDStruct ABCDValue, int ibStart)
        {
        ...
        private uint r1(uint a, uint b, uint c, uint d, uint x, int s, uint t)
        {
        ...
        private uint r2(uint a, uint b, uint c, uint d, uint x, int s, uint t)
        {
        ...
        private uint r3(uint a, uint b, uint c, uint d, uint x, int s, uint t)
        {
        ...
        private uint r4(uint a, uint b, uint c, uint d, uint x, int s, uint t)
        {
        ...
        private uint LSR(uint i, int s)
        {
            return ((i << s) | (i >> (32 - s)));
        }

        //Convert input array into array of UInts
        private uint[] Converter(byte[] input, int ibStart)
        {
            if (null == input)

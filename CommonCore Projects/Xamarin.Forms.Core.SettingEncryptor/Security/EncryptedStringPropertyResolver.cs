...
namespace Xamarin.Forms.Core
{
    ...
    public class EncryptedStringPropertyResolver : DefaultContractResolver
    {
        ...
        protected override IList<JsonProperty> CreateProperties(Type type, MemberSerialization memberSerialization)
        {
            ...
            foreach (JsonProperty prop in props.Where(p => p.PropertyType == typeof(Dictionary<string, string>)))
            {
                PropertyInfo pi = type.GetProperty(prop.UnderlyingName);
                if (pi != null && pi.GetCustomAttribute(typeof(JsonEncryptAttribute), true) != null)
                {
                    prop.ValueProvider = new EncryptedDictionaryValueProvider(pi, encryptionKeyBytes);
                }
            }
        ...
        class EncryptedDictionaryValueProvider: IValueProvider
        {
            PropertyInfo targetProperty;
            private byte[] encryptionKey;
            public EncryptedDictionaryValueProvider(PropertyInfo targetProperty, byte[] encryptionKey)
            {
            ...
            {
                var dict = (Dictionary<string, string>)targetProperty.GetValue(target);
                if (dict == null)
                    return null;
                ...
                foreach (var key in dict.Keys)
                {
                    ...
                    using (AesManaged aes = new AesManaged { Key = encryptionKey })
                    {
                        ...
                        dict[key] = Convert.ToBase64String(outputStream.ToArray());
                    }
                }
            ...
            public void SetValue(object target, object value)
            {
                ...
                foreach (var key in dict.Keys)
                {
                    ...
                    using (AesManaged aes = new AesManaged { Key = encryptionKey })
                    {
                        ...
                        dict[key] = decryptedValue;
                    }
                }

                targetProperty.SetValue(target, dict);
            }
        }
